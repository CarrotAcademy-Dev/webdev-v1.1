...
import { getJenisMerchandise, editJenisMerchandise, deleteJenisMerchandise } from "@/features/hr/gaMainDataApiService";
import {
    Box, Button, Flex, Grid, IconButton, Input, Modal, ModalBody,
    ModalCloseButton, ModalContent, ModalFooter, ModalHeader, ModalOverlay,
    Select, Stack, Text, Textarea, useColorMode, useDisclosure, Badge,
    Skeleton,
} from "@chakra-ui/react";
import { useMutation, useQuery, useQueryClient } from "@tanstack/react-query";
import { useMemo, useState } from "react";
import {
    FiInbox, FiList, FiPlus, FiSearch, FiTrash2, FiGift, FiPackage,
} from "react-icons/fi";
...
const KOLOM_OPTIONS = [
    ...
    { value: "giveaway_merch", label: "Giveaway Merch" },
    { value: "artububurit_giveaway_merch", label: "Artububurit Giveaway Merch" },
];

const JenisMerchandisePage = () => {
    ...
    const editMutation = useMutation({
        mutationFn: editJenisMerchandise,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["jenisMerchandise"] });
            toaster.create({ title: "Berhasil update kolom merchandise", type: "success", duration: 3000 });
        },
        onError: (error) => {
    ...
    const deleteMutation = useMutation({
        mutationFn: deleteJenisMerchandise,
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["jenisMerchandise"] });
        ...
        <ContainerCarrot>
            <StyledDashboardReport colorMode={colorMode}>
                <Box className="header">
                    <h1>Jenis Merchandise</h1>
                    <Text className="subtitle">Kelola daftar item merchandise per kategori paket</Text>
                </Box>
                ...
                {isLoading ? (
                    ...
                    <Flex className="empty-state"><FiInbox size={48} /><Text>Gagal memuat data jenis merchandise</Text></Flex>
                ) : (
                    <Grid templateColumns={{ base: "1fr", lg: "280px 1fr" }} gap={4}>
                        {/* Kolom Selector */}
                        <Box bg={cardBg} border="1px solid" borderColor={borderCol} borderRadius="12px" p={3} h="fit-content">
                            <Text fontWeight="bold" fontSize="sm" mb={3} px={2}>Kategori Merch</Text>
                            <Stack gap={1}>
...
export default JenisMerchandisePage;