...
import { getPengkodeanStock, addPengkodeanStock } from "@/features/hr/stockApiService";
import {
...
const PengkodeanPageStock = () => {
    ...
    const { data: pengkodeanData = [], isLoading, isError } = useQuery({
        queryKey: ["pengkodeanStock"],
        queryFn: getPengkodeanStock,
        staleTime: 5 * 60 * 1000,
        refetchOnWindowFocus: false,
    });

    const addMutation = useMutation({
        mutationFn: addPengkodeanStock,
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ["pengkodeanStock"] });
            toaster.create({
...
export default PengkodeanPageStock;